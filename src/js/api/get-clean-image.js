...
module.exports = function (req, res) {
    Promise.all([
        ...
        .then((results) => {
            ...
            const deadPictures = files.filter((f) =>
                pictures.every((p) => p.url !== "/img/" + f)
            );
            const deadThumbnails = thumbnails.filter((f) =>
                pictures.every((p) => p.thumbnail !== "/thumb/" + f)
            );

            const unmatchedPictures = pictures.filter((p) =>
                files.every((f) => p.url !== "/img/" + f)
            );
            const unmatchedThumbnails = pictures.filter(
                (p) =>
                    p.thumbnail &&
                    thumbnails.every((f) => p.thumbnail !== "/thumb/" + f)
            );
